  ...
  <div class="Coffee">
    <div class="admin-content">
      ...
      <div class="admin-body">
        <h2 class="page-brand">Coffee&nbsp;Shops</h2>
        <div class="table">
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id">
                ...
                <td>
                  ...
                  <router-link
                    :to="'/admin/coffee/products/' + item.id"
                    class="button videos"
                  >
                    Products
                  </router-link>
...
export default {
  ...
  methods: {
    ...
    addItems() {
      this.isLoading = true;
      this.$store
        .dispatch("ADD_COFFEE", this.newItem)
        .then((response) => {
    ...
    updateItems() {
      this.isLoading = true;
      this.$store
        .dispatch("UPDATE_COFFEE", this.selectedItem)
        .then((response) => {
          if (response.data.status == "ok") {
            this.$notify({
              group: "status",
    ...
    deleteItems() {
      this.isLoading = true;
      this.$store
        .dispatch("DELETE_COFFEE", this.selectedItem.id)
        .then((response) => {
          if (response.data.status == "ok") {
            this.$notify({
              group: "status",
