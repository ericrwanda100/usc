  ...
  <div class="articles">
    <div class="admin-content">
      ...
      <div class="admin-body">
        <h2 class="page-brand">Night Clubs</h2>
        <div class="table">
          <div class="table-title">
            <label for="Night Clubs">Night Clubs</label>
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id">
                ...
                <td>
                  ...
                  <router-link
                    :to="'/admin/clubs/products/' + item.id"
                    class="button videos"
                  >
                    Products
                  </router-link>
...
export default {
  ...
  methods: {
    ...
    addItems() {
      this.isLoading = true;
      this.$store
        .dispatch("ADD_CLUBS", this.newItem)
        .then((response) => {
    ...
    updateItems() {
      this.isLoading = true;
      this.$store
        .dispatch("UPDATE_CLUBS", this.selectedItem)
        .then((response) => {
          if (response.data.status == "ok") {
            this.$notify({
              group: "status",
    ...
    deleteItems() {
      this.isLoading = true;
      this.$store
        .dispatch("DELETE_CLUBS", this.selectedItem.id)
        .then((response) => {
          if (response.data.status == "ok") {
            this.$notify({
              group: "status",
